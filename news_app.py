...
# --- 🎨 CSS: Cyber Neon Edition (สมบูรณ์แบบ) ---
st.markdown("""
    <style>
    ...
    .main-title { font-size: 55px; font-weight: 900; text-align: center; background: linear-gradient(to right, #00dfd8, #007cf0, #00dfd8); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 25px; }
    ...
    .news-card { background: rgba(15, 23, 42, 0.85); padding: 20px; border-radius: 15px; margin-bottom: 15px; border-left: 5px solid #00dfd8; box-shadow: 0 10px 20px rgba(0,0,0,0.5); }
    ...
    .btn-news { color: #00dfd8 !important; font-weight: 700; text-decoration: none; font-size: 13px; border: 1px solid #00dfd8; padding: 4px 12px; border-radius: 20px; transition: 0.3s; }
    .btn-news:hover { background: #00dfd8; color: #0f172a !important; }
    ...
    div[data-testid="stFormSubmitButton"] button { background: linear-gradient(45deg, #0070f3, #7928ca, #ff0080) !important; width: 100% !important; font-weight: 900 !important; }
    .low-stock { color: #ff0080 !important; font-weight: 900; }
    </style>
...
if page == "🌐 MARKET INSIGHT":
    # เปลี่ยนไอคอนเป็นรูปโลกเท่ๆ สว่างๆ
    st.markdown('<div style="text-align: center;"><img src="https://cdn-icons-png.flaticon.com/512/4238/4238090.png" width="80"></div>', unsafe_allow_html=True)
    ...
    urls = [
        ("🟡 GOLD NEWS", "https://news.google.com/rss/search?q=gold+price+XAUUSD&hl=en-US", c1),
        ("🟠 CRYPTO NEWS", "https://cointelegraph.com/rss", c2),
        ("🟢 THAI MARKET", "https://news.google.com/rss/search?q=SET50+TFEX&hl=th", c3)
    ]
    
    for title, url, col in urls:
        with col:
...
# =====================================================================
    ...
    # ⚠️ ส่วนแสดงผล: ดึงกลับขึ้นมาวางคู่กัน ⚠️
...
# =====================================================================
    ...
    # ไอคอนสว่างๆ รูปโกดัง
    st.markdown('<div style="text-align: center;"><img src="https://cdn-icons-png.flaticon.com/512/2271/2271068.png" width="80"></div>', unsafe_allow_html=True)
    ...
    with tab_inv:
        if not df_stock.empty:
            ...
            for _, row in df_stock.iterrows():
                if str(row.iloc[0]).strip() == '-': continue
                # ดึงค่าจาก Col H (คงเหลือ) และ Col I (จุดเตือน)
                qty = safe_float(row.iloc[7])
                warn = safe_float(row.iloc[8])
                cls = 'class="low-stock"' if qty <= warn else ""
                html += f'<tr {cls}>' + "".join(f'<td>{v}</td>' for v in row) + '</tr>'
            st.markdown(html + '</tbody></table></div>', unsafe_allow_html=True)

    with tab_out:
        with st.form("out"):
            st.markdown("<p style='text-align:center;'>บันทึกการเบิกสินค้า</p>", unsafe_allow_html=True)
            c1, c2, c3 = st.columns(3)
            hn, name = c1.text_input("HN"), c2.text_input("ชื่อคนไข้")
            ...
            if st.form_submit_button("📤 CONFIRM"):
                try:
                    ...
                    wks.append_row([datetime.now().strftime("%Y-%m-%d"), hn, name, cat, size, color, q], value_input_option="USER_ENTERED")
                    st.success("SUCCESS!"); st.cache_data.clear(); time.sleep(1); st.rerun()
                except Exception as e: st.error(e)

    with tab_in:
        with st.form("in"):
            st.markdown("<p style='text-align:center;'>บันทึกรับเข้าคลัง</p>", unsafe_allow_html=True)
            c1, c2, c3 = st.columns(3)
            cat_in = c1.selectbox("หมวดหมู่สินค้า", items)
            size_in, color_in = c2.text_input("ไซส์ (IN)"), c3.text_input("สี (IN)")
            q_in, cost = st.number_input("จำนวนที่รับเข้า", min_value=1), st.number_input("ต้นทุนรวม")
            if st.form_submit_button("📥 CONFIRM"):
                try:
