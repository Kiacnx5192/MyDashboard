...
st.markdown("""
    ...
    [data-testid="stVerticalBlockBorderWrapper"] { background: rgba(15, 23, 42, 0.7) !important; backdrop-filter: blur(15px) !important; border: 1px solid rgba(56, 189, 248, 0.4) !important; border-radius: 15px !important; padding: 20px !important; box-shadow: 0 0 25px rgba(56, 189, 248, 0.15) !important; }
    ...
    .main-title { font-size: 45px; font-weight: 900; text-align: center; background: linear-gradient(to right, #00dfd8, #007cf0, #00dfd8); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 20px; }
    .sub-header { color: #f472b6; text-align: center; font-size: 20px; font-weight: 800; margin-bottom: 15px; text-transform: uppercase; letter-spacing: 1.5px; text-shadow: 0 0 10px rgba(244, 114, 182, 0.3);}
    .table-wrapper { height: 400px; overflow-y: auto; overflow-x: auto; border-radius: 12px; background: rgba(0,0,0,0.4); border: 1px solid rgba(56, 189, 248, 0.2); }
    .custom-table { width: 100%; border-collapse: collapse; min-width: 900px; } 
    .custom-table th { background: #0f172a; color: #00dfd8; padding: 12px; position: sticky; top: 0; z-index: 10; border-bottom: 2px solid #007cf0; font-size: 12px; text-transform: uppercase;}
    .custom-table td { padding: 10px; text-align: center !important; color: #f8fafc; border-bottom: 1px solid rgba(56, 189, 248, 0.1); font-size: 13px; white-space: nowrap;}
    .news-card { background: rgba(15, 23, 42, 0.85); padding: 15px; border-radius: 12px; margin-bottom: 12px; box-shadow: 0 8px 15px rgba(0,0,0,0.5); border-left: 4px solid #00dfd8; }
    div[data-testid="stMetricValue"] > div { color: #00dfd8 !important; font-size: 36px !important; font-weight: 900 !important; }
    div[data-testid="stFormSubmitButton"] button { background: linear-gradient(45deg, #0070f3, #7928ca, #ff0080) !important; width: 100% !important; font-weight: 900 !important; height: 45px !important;}
    .low-stock { color: #ff0080 !important; font-weight: 900; text-shadow: 0 0 8px rgba(255, 0, 128, 0.5); }
    </style>
...
def load_any_sheet(sheet_name_hint):
    try:
        gc = get_gspread_client()
        sh = gc.open_by_key("1uxDki739Juxrsu1HfYZAsmDVZETRtd-1liaw6LT8P8w")
        # ค้นหา Sheet ที่มีชื่อใกล้เคียง (ยืดหยุ่นขึ้น)
        all_sheets = [s.title for s in sh.worksheets()]
        target_name = next((s for s in all_sheets if sheet_name_hint in s), None)
        if not target_name: return pd.DataFrame()
        data = sh.worksheet(target_name).get_all_values()
...
if page == "🌐 MARKET INSIGHT":
    ...
    urls = [("🟡 GOLD", "https://news.google.com/rss/search?q=gold+price&hl=en-US"), 
            ("🟠 CRYPTO", "https://cointelegraph.com/rss"), 
            ("🟢 THAI", "https://news.google.com/rss/search?q=SET50&hl=th")]
    for col, (title, url) in zip([c1, c2, c3], urls):
        with col:
            st.markdown(f"<h3 style='text-align:center;'>{title}</h3>", unsafe_allow_html=True)
            f = feedparser.parse(url)
            for e in f.entries[:3]:
                st.markdown(f'<div class="news-card"><strong>{e.title}</strong><br><a href="{e.link}" target="_blank" style="color:#00dfd8; font-size:12px;">READ STORY</a></div>', unsafe_allow_html=True)
...
# =====================================================================
    ...
    with st.container(border=True):
        ...
        with t2:
            st.markdown("<p style='color:#fde047; text-align:center;'>แก้ไขไม้เทรด</p>", unsafe_allow_html=True)
            if not df_log.empty:
                ids = [str(x) for x in df_log.iloc[:, 0].tolist() if str(x).strip() not in ['', '-']]
                sel_id = st.selectbox("เลือก ID ไม้:", ["-- เลือก --"] + ids)
                if sel_id != "-- เลือก --":
                    row_idx = df_log[df_log.iloc[:, 0].astype(str) == sel_id].index[0]
                    with st.form("edit_trade"):
                        ec1, ec2 = st.columns(2)
                        new_res = ec1.selectbox("Result", ["Win", "Loss", "กันทุน", "Pending"], index=0)
                        new_pl = ec2.number_input("P/L ($)", value=safe_float(df_log.iloc[row_idx, 8]))
            ...
            st.divider()
            conf = st.text_input("พิมพ์ RESET เพื่อล้าง Data8:")
            if st.button("🧨 RESET ALL DATA", type="primary", disabled=(conf != "RESET")):
                try:
                    gc = get_gspread_client(); gc.open_by_key("1uxDki739Juxrsu1HfYZAsmDVZETRtd-1liaw6LT8P8w").worksheet("Data8").batch_clear(["A4:Q1000"])
                    st.success("RESET SUCCESS!"); st.cache_data.clear(); time.sleep(1); st.rerun()
                except: st.error("Error")
    ...
    with col_l:
        ...
        df_dash = load_any_sheet("Dashboard8")
...
# =====================================================================
    ...
    # ดึงข้อมูลจาก Master_Stock
    df_stk = load_any_sheet("Master_Stock")
    ...
    with tab_inv:
        if not df_stk.empty:
            ...
            st.warning("⚠️ ไม่พบแผ่นงาน 'Master_Stock' หรือไม่มีข้อมูล กรุณาเช็กชื่อ Sheet ในไฟล์ Google Sheets อีกครั้งนะคะ")
    ...
    with tab_out:
        with st.form("out_form"):
            ...
            sz, cl, q = c4.text_input("ไซส์"), c5.text_input("สี"), c6.number_input("จำนวนเบิก", min_value=1)
    ...
    with tab_in:
        with st.form("in_form"):
            ...
            q_in, cst = st.number_input("จำนวนรับเข้า", min_value=1), st.number_input("ต้นทุนรวม")
            if st.form_submit_button("📥 CONFIRM"):
                try:
                    gc = get_gspread_client(); wks = gc.open_by_key("1uxDki739Juxrsu1HfYZAsmDVZETRtd-1liaw6LT8P8w").worksheet("Log_รับเข้า")
                    wks.append_row([datetime.now().strftime("%d/%m/%Y"), cat_in, sz_in, cl_in, q_in, cst], value_input_option="USER_ENTERED")
                    st.success("SUCCESS!"); st.cache_data.clear(); time.sleep(1); st.rerun()
                except: st.error("Error")
